...
{
	int zeile = 100, spalte = 100, zeile_neu = 100, spalte_neu = 100, abbruch = 0;
	string auswahl_stein = "", auswahl_feld = "";

while (zeile_neu > 7 || spalte_neu > 7)
{
	while (zeile > 7 || spalte > 7)
	{
		...
		if (zeile < 8 && spalte < 8)
		{
			if (brett->getFeld(spalte,zeile)->getBesetzt() == false || brett->getFeld(spalte,zeile)->getGast()->getMteam()->getFarbe() == farbe)
			{
				zeile = 100;
				spalte = 100;
				system("cls");
				cout << "\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\t An dieser Position befindet sich kein Stein bzw. dieser Stein geh\224rt nicht zu ihrem Team!!!";
				Sleep(3000);
			};
		} else
		{
			system("cls");
			cout << "\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\t Ihre Eingabe entsprach nicht der Vorgabe!!!";
			Sleep(3000);
		}
	}
	...
	cout << "\n Wohin soll der Stein ziehen? (Zeile,Spalte): ";
	cin	>> auswahl_stein;
	cin.sync();
	...
	if (spalte_neu < 8 && zeile_neu < 8)
	{
	for (int zaehler = 0; zaehler < brett->getFeld(spalte,zeile)->getGast()->zuege().size(); zaehler++)
	{
		if (brett->getFeld(spalte,zeile)->getGast()->zuege()[zaehler] == brett->getFeld(spalte_neu,zeile_neu)) {abbruch = 1;};
	}
	if (abbruch == 0)
	{
		zeile_neu = 100;
		...
		spalte = 100;
		system("cls");
		cout << "\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\tZu dieser Position kann der Stein nicht springen!!!";
		Sleep(3000);
	}
	} else
	{
...
void GUI::zeichneSpielfeld(int zug,int spieler)
{
	...
	cout <<	"\n\n Es ist der: "
		 ...
		 << spieler
		 << " (";
	if (spieler == 1) { cout << " (Wei\341)"; } else { cout << " (Schwarz)";}
